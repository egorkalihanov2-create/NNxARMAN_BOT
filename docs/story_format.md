...
Когда следующий сегмент имеет другой `mini_segment`, бот пакетно удаляет сообщения предыдущего мини-сегмента, делает короткую паузу и показывает новый блок. Команда `/start` не добавляется в отслеживаемые сообщения, чтобы чат не становился полностью пустым.
...
Авто-сегменты без кнопки выдерживаются на экране минимум 5 секунд; длинные блоки могут висеть дольше.

## Реплика игрока

Чтобы показать реплику игрока без имени и без синей Telegram-реплики, используется `player_line`:
